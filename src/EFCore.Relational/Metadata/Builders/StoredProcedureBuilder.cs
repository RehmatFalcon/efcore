...
public class StoredProcedureBuilder : IInfrastructure<EntityTypeBuilder>, IInfrastructure<IConventionStoredProcedureBuilder>
{
    ...
    {
        Builder.HasParameter(propertyName, ConfigurationSource.Explicit);
        buildAction(new(CreateIdentifier(), CreatePropertyBuilder(propertyName)));
        return this;
    }

    private StoreObjectIdentifier CreateIdentifier()
    {
        if (Metadata.EntityType.GetInsertStoredProcedure() == Metadata)
        {
            return StoreObjectIdentifier.InsertStoredProcedure(Metadata.Name, Metadata.Schema);
        }
        
        if (Metadata.EntityType.GetDeleteStoredProcedure() == Metadata)
        {
            return StoreObjectIdentifier.DeleteStoredProcedure(Metadata.Name, Metadata.Schema);
        }

        Check.DebugAssert(Metadata.EntityType.GetUpdateStoredProcedure() == Metadata,
            "Unexpected stored procedure type");

        return StoreObjectIdentifier.UpdateStoredProcedure(Metadata.Name, Metadata.Schema);
    }
